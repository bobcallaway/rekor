...
func (t *TrillianClient) ensureStarted(ctx context.Context) error {
	...
	if _, ok := ctx.Deadline(); !ok {
		cctx, cancel = context.WithTimeout(ctx, initLatestRootTimeout)
	}
	if cancel != nil {
		defer cancel()
	}
...
func (t *TrillianClient) updater() {
	for {
		// allow periodic wake to check stop
		ctx, cancel := context.WithTimeout(t.bgCtx, updaterWaitTimeout)
		...
		if err != nil {
			log.Logger.Debugw("trillian root update wait error", "treeID", t.logID, "err", err)
