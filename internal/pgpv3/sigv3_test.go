...
func v3Fixture(t *testing.T) (*Signature, []byte, openpgp.EntityList) {
	...
	sigPacket, err := os.ReadFile("../../pkg/pki/pgp/testdata/repomd.xml.sig")
	if err != nil {
		t.Fatal(err)
	}
	...
	data, err := os.ReadFile("../../pkg/pki/pgp/testdata/repomd.xml")
	if err != nil {
		t.Fatal(err)
	}
	keyData, err := os.ReadFile("../../pkg/pki/pgp/testdata/repomd_armored_public.pgp")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeclaredHashesAreLinked(t *testing.T) {
	for _, id := range []byte{1, 2, 8, 9, 10, 11, 12, 14} {
		h, ok := algorithm.HashIdToHashWithSha1Md5(id)
		if !ok {
			t.Fatalf("unknown hash id %d", id)
		}
		if !h.Available() {
...
func TestParse(t *testing.T) {
	...
	tests := []struct {
		...
		{"trailing data", func(body []byte) []byte { return append(body, 0) }},
	}
	...
	for n := 0; n < len(op.Contents); n++ {
		if _, err := Parse(op.Contents[:n]); err == nil {
