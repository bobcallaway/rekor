...
func init() {
	...
	rootCmd.PersistentFlags().Duration("trillian_log_server.root_rpc_timeout", trillianclient.DefaultRootRPCTimeout, "per-RPC timeout applied to every GetLatestSignedLogRoot call (initialization, background updater polls, and on-demand refreshes) when cache_sth is enabled; bounds how long a hung Trillian call can stall cache progress")
	rootCmd.PersistentFlags().Duration("trillian_log_server.sth_poll_interval", trillianclient.DefaultPollInterval, "polling interval at which the background updater fetches the latest tree head when cache_sth is enabled; also the minimum spacing between on-demand refreshes and between initialization attempts. Polling faster than Trillian's sequencer_interval does not lower inclusion latency; values below "+trillianclient.MinPollInterval.String()+" are clamped")
	rootCmd.PersistentFlags().Duration("trillian_log_server.max_sth_staleness", 0, "maximum age of an active cached tree head before reads refetch it synchronously and fail with 503 if that does not restore freshness; applies to log-info, inclusion proofs, and entry upload alike. Zero derives three polling intervals plus root_rpc_timeout (under 4s at the defaults), so a Trillian brownout lasting more than a few seconds fails reads rather than serving an uncorroborated root; raise it to trade correctness for availability")
