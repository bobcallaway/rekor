...
var serveCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for kind, versionList := range pluggableTypeMap {
			log.Logger.Infof("Loading support for pluggable type '%v'", kind)
			for _, version := range versionList {
				log.Logger.Infof("Loading version '%v' for pluggable type '%v'", version, kind)
			}
		}
